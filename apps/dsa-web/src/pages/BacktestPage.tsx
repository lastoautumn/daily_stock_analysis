...
const PerformanceCard: React.FC<{ metrics: PerformanceMetrics; title: string }> = ({ metrics, title }) => (
  <Card variant="gradient" padding="md" className="animate-fade-in">
    ...
    <MetricRow label="平均实际涨幅" value={pct(metrics.avgStockReturnPct)} />
    ...
    <div className="mt-3 pt-2 border-t border-white/5 flex items-center justify-between">
      <span className="text-xs text-muted">评估数</span>
      <span className="text-xs text-secondary font-mono">
        {Number(metrics.completedCount)} / {Number(metrics.totalEvaluations)}
      </span>
    </div>
    <div className="flex items-center justify-between">
      <span className="text-xs text-muted">盈 / 亏 / 平</span>
...
const RunSummary: React.FC<{ data: BacktestRunResponse }> = ({ data }) => (
  <div className="flex items-center gap-4 px-3 py-2 rounded-lg bg-elevated border border-white/5 text-xs font-mono animate-fade-in">
    ...
    <span className="text-secondary">已完成: <span className="text-emerald-400">{data.completed}</span></span>
      ...
      <main className="flex-1 flex overflow-hidden p-3 gap-3">
        ...
        <div className="flex flex-col gap-3 w-64 flex-shrink-0 overflow-y-auto">
          {isLoadingPerf ? (
            ...
            <PerformanceCard metrics={overallPerf} title="综合绩效" />
          ) : (
            <Card padding="md">
              <p className="text-xs text-muted text-center py-4">
                暂无回测数据，点击「运行回测」开始评估历史分析准确率。
              </p>
            </Card>
          )}

          {stockPerf && (
            <PerformanceCard metrics={stockPerf} title={`${stockPerf.code || codeFilter} 绩效`} />
          )}
        </div>
        ...
        <section className="flex-1 overflow-y-auto">
          {isLoadingResults ? (
            <div className="flex flex-col items-center justify-center h-64">
              ...
              <p className="mt-3 text-secondary text-sm">加载结果中...</p>
            </div>
          ) : results.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-64 text-center">
              ...
              <h3 className="text-base font-medium text-white mb-1.5">暂无结果</h3>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-elevated text-left">
                      ...
                      <th className="px-3 py-2.5 text-xs font-medium text-secondary uppercase tracking-wider">日期</th>
                      ...
                      <th className="px-3 py-2.5 text-xs font-medium text-secondary uppercase tracking-wider text-right">收益%</th>
